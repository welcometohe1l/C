...
int tpool_add_work(tpool_t tpool, void *routie, void *arg) {
    tpool_work_t *workp;
    pthread_mutex_lock(&tpool->queue_lock);
...
int tpool_destroy(tpool_t tpool, int finish) {
    ...
    /* If the finnish flag is set, wait for workers ro drain queue */
    if (finish == 1) {
